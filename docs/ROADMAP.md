...
- [x] **M26a — Interactive cameras (Fly + Orbit, ImGuiIO-driven)**
  ...
  - [x] **`engine/scene/camera_controller.{h,cpp}`** — two concrete controllers, no virtual dispatch.
    ...
    - `OrbitCameraController` — Maya-style with macOS trackpad ergonomics. `Option+LMB` orbits, `Option+MMB` *or* `Option+Shift+LMB` pans (the Shift path is the trackpad-friendly fallback — Macs don't expose an MMB), `Option+RMB` drag or scroll wheel dollies in/out (no modifier needed for scroll; 2-finger drag on a trackpad reaches the scroll path). State is `{pivot, yaw, pitch, distance}`; eye + view derived each frame.
  ...
  - [x] **Demo**: holds both controllers + a `CamMode` enum (default Orbit). Each loop iteration, if `editor` exists, reads `editor->input_state()`, handles `F1` to toggle mode (re-syncs the active controller from the live camera), then runs the active controller's `update(camera, in_state, dt)` before building the FG. 1-frame input→camera lag, imperceptible at 60+ fps. Tab was the obvious choice initially but ImGui's `NavEnableKeyboard` claims ownership of Tab once any widget gains focus — `IsKeyDown(Tab)` silently filters for non-owners and the second press never reaches us. F1 has no such conflict.
