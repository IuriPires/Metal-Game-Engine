...
RenderEncoder::RenderEncoder(RenderEncoder&& other) noexcept : native_(other.native_) {
...
void RenderEncoder::draw(std::uint32_t vertex_count, std::uint32_t instance_count,
    ...
    auto* enc = static_cast<MTL::RenderCommandEncoder*>(native_);
    // We carry topology on the pipeline; default to TriangleList here. Render
    // graph (M5+) will use the pipeline-bound topology.
    enc->drawPrimitives(MTL::PrimitiveTypeTriangle, base_vertex, vertex_count,
